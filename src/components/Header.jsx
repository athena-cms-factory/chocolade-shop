...
function Header({ primaryTable, tableName, siteSettings = {} }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[1000] bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-4">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 bg-accent rounded-xl flex items-center justify-center text-white font-serif font-bold text-xl shadow-lg shadow-accent/20 group-hover:scale-110 transition-transform">
            {info.site_naam?.charAt(0) || 'A'}
          </div>
          <span className="text-xl font-serif font-bold tracking-tight text-primary">
            <EditableText value={info.site_naam || 'Athena Shop'} cmsBind={{file: 'basisgegevens', index: 0, key: 'site_naam'}} />
          </span>
        </Link>

        {/* Navigatie & Cart */}
        <div className="flex items-center gap-8">
          <a href="#producten" className="hidden md:block font-medium hover:text-accent transition-colors">Onze Pralines</a>
